...
| `esp32-s3-hybrid` | HybridDevKit | 灌溉+摄像头（USB/DVP） |
...
- **Atlas 200I DK A2 版**: 使用USB摄像头，通过OpenCV `cv2.VideoCapture()` 读取，即插即用
- **ESP32版**: 使用DVP接口摄像头（OV2640/OV5640），通过`esp_camera`库读取

> Atlas版使用USB摄像头，无需DVP引脚配置。ESP32版的DVP引脚配置见 `AppConfig.h`。

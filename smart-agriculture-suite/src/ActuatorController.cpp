...
void ActuatorController::setManualValve(bool on) {
    ...
    if (!status_.autoMode) {
        applyOutputs(on, ControlSource::Manual, 0);
    }
}
...
bool ActuatorController::startTimedRun(ControlSource source, int durationSec, unsigned long nowMs) {
    ...
    applyOutputs(true, source, untilMs);
...
void ActuatorController::stopTimedRun() {
    ...
    applyOutputs(false, ControlSource::None, 0);
}
...
void ActuatorController::update(bool baseAutoRequest, unsigned long nowMs) {
    ...
    // 手动模式
    if (!status_.autoMode) {
        applyOutputs(status_.manualValve, ControlSource::Manual, 0);
        return;
    }

    // 自动模式 - 规则引擎
    if (baseAutoRequest) {
        applyOutputs(true, ControlSource::Fallback, 0);
    } else {
        applyOutputs(false, ControlSource::None, 0);
    }
}
...
void ActuatorController::applyOutputs(bool valveOn, ControlSource source, unsigned long untilMs) {
    // 驱动BTS7960 (仅阀门)
